...
| Signals/prediction | ML model from the start | Gradient boosting on engineered features; **walk-forward validation only** — no lookahead. **Known limitation (#25, 2026-07-17):** prediction-time liquidity floor (median 20-bar volume ≥ 100k shares) excludes thin names from rankings while labels/training keep them — trained base rates include moves the ranking will never surface |
